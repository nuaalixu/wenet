...
def write_tar_file(data_list,
    ...
    with tarfile.open(tar_file, "w") as tar:
        for item in data_list:
            ...
            key, txt, wav = item
            suffix = wav.split('.')[-1]
            assert re.fullmatch(r'ark:[0-9]+', suffix)
            ...
            suffix = "feat" if use_feat else "wav"
            wav_file = key + '.' + suffix
            wav = wav.encode('utf8')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_feat',
                        action='store_true',
                        help='use precomputed features')
    parser.add_argument('wav_file', help='wav scp or feat scp')
    ...
    if use_feat:
        logging.info("use precomputed features, ensure the input file is feats.scp")
    ...
    for i, chunk in enumerate(chunks):
        ...
        pool.apply_async(
            write_tar_file,
            (chunk, tar_file, i, num_chunks, use_feat))
